...
function buildEmailHtml(data: {
  ...
  const pathMatch = pagePath.match(/\/garage-door\/([^/]+)/);
  ...
  const sourceLabel = isHomePage ? 'Homepage' : `${cityDisplay} — Garage Door Repair`;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const html = buildEmailHtml({
    siteName: site_name || 'Garage Door Repair',
    fullName,
    email,
    phone: phone || '',
    comment: comment || '',
  ...
  try {
    await resend.emails.send({
      from: 'NKY Garage Door Alerts <alerts@mursenmaintenance.com>',
